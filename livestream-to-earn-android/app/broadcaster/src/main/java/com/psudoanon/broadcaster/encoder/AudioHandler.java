...
public class AudioHandler extends Handler {
    ...
    public boolean startAudioEncoder(IMediaMuxer muxerHandler, int sampleRate, int bufferSize, int bitrate) {
        boolean result = false;

        audioEncoder = new AudioEncoder();
        try {
            result = audioEncoder.startAudioEncoder(sampleRate, 1, bitrate, bufferSize, muxerHandler);
        } catch (Exception e) {
            e.printStackTrace();
            audioEncoder = null;
        }
